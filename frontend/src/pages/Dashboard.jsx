...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    api
      ...
      .catch(() => active && setData({ stats: {}, recent: [] }))
    ...
    <div style={{ maxWidth: 1080 }}>
      <div style={{ marginBottom: 22 }}>
        ...
        <p style={{ margin: 0, fontSize: 15, color: muted(66) }}>
          {s.solved ? `You've solved ${s.solved} of ${s.totalProblems} problems. Keep the streak going.` : "Solve your first problem to get started."}
        </p>
      </div>

      <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 14, marginBottom: 18 }}>
        <Stat label="Problems solved" value={s.solved ?? 0} note={`of ${s.totalProblems ?? 0}`} valueColor="var(--color-accent-2-700)" />
        <Stat label="Submissions" value={s.submissions ?? 0} note={`${s.accepted ?? 0} accepted`} />
        <Stat label="Acceptance" value={`${s.acceptanceRate ?? 0}%`} note="of your submissions" />
        <Stat label="Available" value={s.totalProblems ?? 0} note="published problems" valueColor="var(--color-accent)" />
      </div>
              ...
              {recent.map((r, i) => {
                ...
                return (
                  <Link key={r.id} to={r.slug ? `/problems/${r.slug}` : "#"} className="hover-row" style={{ display: "flex", alignItems: "center", gap: 12, padding: "9px 6px", textDecoration: "none", color: "inherit", borderBottom: i < recent.length - 1 ? "1px solid color-mix(in srgb, var(--color-text) 7%, transparent)" : "none" }}>
                    ...
                    <span style={{ fontSize: 11, padding: "2px 9px", borderRadius: 999, background: d.bg, color: d.fg, fontWeight: 600 }}>{d.label}</span>
